...
int main(int argc, char **argv)
{
  ...
  IMP::Particles residue_particles = IMP::atom::get_by_type(mhd, IMP::atom::RESIDUE_TYPE);

  std::cerr << "Psi of the first residue " << pk.get_psi(IMP::atom::Residue(residue_particles[0])) << std::endl;
  pk.set_psi(IMP::atom::Residue(residue_particles[0]), 3.14);

  std::cerr << "Psi of the first residue after change" << pk.get_psi(IMP::atom::Residue(residue_particles[0])) << std::endl;
